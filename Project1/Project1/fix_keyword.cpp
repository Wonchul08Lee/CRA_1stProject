...
void calPointPerfectHit(vector<Node2>& bestList, const string& w, long long int& max, int& flag) {
	for (Node2& node : bestList) {
		if (node.name == w) {		
			max = node.point + static_cast<int>(node.point * 0.1); // 10% 증가
...
void addAndSortKeyword(vector<Node2>& list, const string& w, int point, size_t maxSize = 10) {
	...
	else if (!list.empty() && list.back().point < point) {	// 요일별 10개 keywork 넘으면
		list.pop_back();
		list.push_back({ w, point });
		std::sort(list.begin(), list.end());
	}
...
string recommandKeyword(string keyword, string weekday) {
...
vector<pair<string, string>> readInputFile(const string& filename) {
...
// 입력 벡터를 받아 결과 벡터를 반환
vector<string> processKeywords(const vector<pair<string, string>>& data) {
...
void loadAndProcessKeyword(const std::string& filename) {
	auto data = readInputFile(filename);
